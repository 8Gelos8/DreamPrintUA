      ...
      <div className="absolute top-8 left-8 z-10 max-w-xs pointer-events-none">
         ...
         <div className="mt-2 flex items-center gap-3 pointer-events-auto">
             <p className="text-stone-400 font-bold text-sm uppercase tracking-wider flex items-center gap-2">
                {loading ? 'Оновлення...' : 'Клікай на фото!'}
                {!loading && items.length > 0 && <FolderOpen size={16} />}
             </p>
             <button 
      ...
      {items.map((item, index) => {
        ...
        return (
          <motion.div
            ...
            className={`absolute cursor-pointer shadow-lg hover:shadow-2xl transition-shadow bg-white p-2 md:p-3 pb-8 md:pb-12 border-b-4 ${borderColor}`}
            initial={{ 
              opacity: 0,
              scale: 0.5,
              top: '50%', 
