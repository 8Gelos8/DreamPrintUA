...
const DesktopGallery: React.FC = () => {
  ...
  return (
    <div className="relative w-full h-[600px] md:h-[700px] overflow-hidden bg-gradient-to-br from-stone-50 to-stone-100 rounded-3xl shadow-inner border-4 border-white perspective-1000 group">
      {/* Desk texture hint */}
      ...
      {allItems.length > 0 && (
        <div className="absolute inset-0 flex items-center justify-center">
          {/* Large Center Photo */}
          <motion.div
            key={`large-${allItems[activeIndex]?.id}`}
            layoutId={`large-card-${allItems[activeIndex]?.id}`}
            initial={{ scale: 0.8, opacity: 0 }}
            animate={{ scale: 1, opacity: 1 }}
            exit={{ scale: 0.8, opacity: 0 }}
            transition={{ duration: 0.5, type: "spring" }}
            onClick={() => setSelectedId(allItems[activeIndex]?.id || null)}
            className="absolute w-64 md:w-96 cursor-pointer shadow-2xl hover:shadow-3xl transition-shadow bg-white p-3 md:p-4 pb-12 md:pb-16 border-b-4 border-dream-cyan group"
          >
            {/* Tape effects */}
            <div className="absolute -top-4 left-1/4 w-8 h-12 bg-white/40 backdrop-blur-sm shadow-sm -rotate-12 border-l border-r border-white/50"></div>
            <div className="absolute -top-4 right-1/4 w-8 h-12 bg-white/40 backdrop-blur-sm shadow-sm rotate-12 border-l border-r border-white/50"></div>

            <div className="aspect-square w-56 md:w-80 bg-stone-100 overflow-hidden mb-4 border border-stone-100 relative group/img">
              <img 
                src={allItems[activeIndex]?.imageUrl} 
                alt={allItems[activeIndex]?.title}
            ...
            <div className="font-handwriting text-center text-base md:text-lg text-stone-700 font-bold font-display line-clamp-2 capitalize break-words">
              {allItems[activeIndex]?.title}
            </div>
          </motion.div>

          {/* Scattered Photos Behind - Randomized Positions */}
          {allItems.map((item, index) => {
            const borderColor = COLORS[index % COLORS.length];
            const isUserPhoto = userPhotos.some(p => p.id === item.id);
            
            // Don't show active item in background
            if (index === activeIndex) return null;
            
            // Deterministic randomization based on index
            const seed = item.id.charCodeAt(0) + index;
            const offsetX = ((seed * 7) % 300) - 150;
            const offsetY = ((seed * 11) % 200) - 100;
            const rotation = ((seed * 13) % 45) - 22;
            const scale = 0.3 + ((seed % 3) * 0.1);
            const zIndex = Math.floor(seed / 100);
            ...
            return (
              <motion.div
                ...
                onClick={() => {
                  setActiveIndex(allItems.indexOf(item));
                  setSelectedId(null);
                }}
                initial={{ opacity: 0, scale: 0.5 }}
                animate={{ opacity: 0.6, scale: scale }}
                whileHover={{ opacity: 1, scale: scale + 0.1, zIndex: 40 }}
                transition={{ duration: 0.4 }}
                className={`absolute cursor-pointer shadow-lg hover:shadow-2xl transition-all bg-white p-2 border-b-4 ${borderColor}`}
                style={{
                  maxWidth: '180px',
              ...
              >
                ...
                {isUserPhoto && (
                  <button
                    ...
                    className="absolute -top-2 -right-2 p-1 bg-red-500 rounded-full text-white shadow-lg opacity-0 hover:opacity-100 transition-opacity z-10 hover:bg-red-600"
                    title="Видалити фото"
                  >
                    <X size={14} />
                  </button>
                ...
                <div className="aspect-square w-32 bg-stone-100 overflow-hidden mb-2 border border-stone-100 relative group/img">
                  <img src={item.imageUrl} alt={item.title} className="w-full h-full object-cover pointer-events-none" />
                  <div className="absolute inset-0 bg-gradient-to-tr from-white/0 via-white/30 to-white/0 opacity-0 group-hover/img:opacity-100 transition-opacity duration-500"></div>
                </div>
                <div className="font-handwriting text-center text-xs md:text-sm text-stone-700 font-bold font-display line-clamp-1 capitalize break-words">
                  {item.title}
                </div>
              </motion.div>
            );
      ...
      <div 
        ...
        className="absolute bottom-0 left-0 right-0 h-32 bg-gradient-to-t from-stone-900/20 to-transparent overflow-x-auto overflow-y-hidden scroll-smooth snap-x snap-mandatory"
        onScroll={(e) => {
          const container = e.currentTarget;
          const scrollLeft = container.scrollLeft;
          const itemWidth = 140;
          const newIndex = Math.round(scrollLeft / itemWidth);
          setActiveIndex(Math.max(0, Math.min(newIndex, allItems.length - 1)));
        }}
      >
        <div className="flex gap-3 p-4 min-w-max h-full items-center">
          {allItems.map((item, index) => {
            ...
            return (
              <motion.div
                ...
                className={`flex-shrink-0 cursor-pointer rounded-lg overflow-hidden transition-all ${
                  isActive
                    ? 'ring-4 ring-dream-cyan shadow-lg w-28 h-28'
                    : 'opacity-60 hover:opacity-100 w-24 h-24'
                }`}
      ...
      {allItems.length === 0 && !loading && (
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-stone-500 text-sm">Завантажте свої первші фото в адмінпанелі!</p>
          </div>
        </div>
      )}
      ...
      <AnimatePresence>
        {selectedId && selectedItem && (
          <div className="absolute inset-0 z-50 flex items-center justify-center p-4 bg-stone-900/60 backdrop-blur-md" onClick={() => setSelectedId(null)}>
            <motion.div
              layoutId={`large-card-${selectedId}`}
