...
import { Lock, Home, ShoppingBag, DollarSign, Image } from 'lucide-react';
...
type Tab = 'photos' | 'texts' | 'products' | 'prices' | 'settings';
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="flex flex-wrap gap-2 mb-8 bg-white rounded-2xl p-2 shadow-lg">
        {[
          { id: 'photos' as Tab, label: 'Фото', icon: <Image size={20} /> },
          { id: 'texts' as Tab, label: 'Тексти', icon: <Home size={20} /> },
          { id: 'products' as Tab, label: 'Продукти', icon: <ShoppingBag size={20} /> },
          { id: 'prices' as Tab, label: 'Ціни', icon: <DollarSign size={20} /> },
          { id: 'settings' as Tab, label: 'GitHub', icon: <Lock size={20} /> },
        ].map(tab => (
          <button
            key={tab.id}
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg font-bold transition-all ${
              activeTab === tab.id
                ? 'bg-gradient-to-r from-dream-purple to-dream-pink text-white shadow-md'
